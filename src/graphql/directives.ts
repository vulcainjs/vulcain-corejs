...
//
// from https://github.com/lirown/graphql-custom-directive
//
 ...
 */
const GraphQLCustomDirective = function (config) {
...
export const PaginateDirective = GraphQLCustomDirective({
    name: 'paginate',
    description:
        'Paginate results',
    locations: [
    ...
    args: { page: { type: GraphQLNonNull(GraphQLInt) }, pageSize: { type: GraphQLInt } },
    async resolve(resolve, source, args, context) {
        let oldPage = context.requestData.page;
        let oldPageSize = context.requestData.pageSize;
        
        if (args.page)
            context.requestData.page = parseInt(args.page);
        if (args.pageSize)
            context.requestData.pageSize = parseInt(args.pageSize);
        
        let res = await resolve();

        context.requestData.page = oldPage;
        context.requestData.pageSize = oldPageSize;
        return res;
    }
});